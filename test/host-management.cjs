...
const testUrl = process.env.NET_TEST_URL || 'http://127.0.0.1:5174/';
  ...
  try {
    for (const width of [1280, 390]) {
      ...
      let state = { available: true, powerAvailable: true, updateState: 'available', checkedAt: new Date().toISOString(), checking: false,
        release: { version: '0.2.1', backend: 'a'.repeat(40), frontend: 'b'.repeat(40), notes: ['Opravy monitoringu', 'Nové bezpečné ovládání hostitele'] },
        operation: { state: 'idle' }, history: [{ version: '0.2.0', state: 'succeeded', at: new Date().toISOString(), notes: ['Historická změna'] }] };
      const calls = [];
      ...
      await page.route('**/api/v1/**', async route => {
        ...
        if (url.pathname.includes('/host-control')) {
          ...
          if (req.method() === 'POST') {
            ...
            state = { ...state, operation: action === 'install' ? { state: 'installing' } : action === 'cancel-power' ? { state: 'cancelled' } :
              { state: 'scheduled', action, delayMinutes: body.delayMinutes, cancellable: body.delayMinutes > 0 } };
          }
      ...
      await page.getByRole('button', { name: 'Podrobnosti NET 0.2.0' }).click();
      await page.getByText('Historická změna', { exact: true }).waitFor();
      ...
      const installButton = modal.getByRole('button', { name: 'Instalovat aktualizaci', exact: true });
      const installBox = await installButton.boundingBox();
      await page.mouse.move(installBox.x + installBox.width / 2, installBox.y + installBox.height / 2);
      ...
      assert.equal(calls[1].delayMinutes, 5);
      await page.getByRole('button', { name: 'Zrušit naplánovanou akci', exact: true }).click();
      await modal.getByLabel('Administrační klíč', { exact: true }).fill('x'.repeat(64));
      const cancelButton = modal.getByRole('button', { name: 'Zrušit naplánovanou akci', exact: true });
      ...
      assert.equal(calls[2].action, 'cancel-power');
      assert.equal(calls[2].confirmation, undefined);
      await page.reload();
      await page.getByRole('button', { name: 'Nastavení', exact: true }).click();
